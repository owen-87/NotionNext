...
> 审阅日期：2026-07-27
> 最近更新：2026-08-11
> 当前状态：批次 A、B、C 已部署；批次 D 模板代码已提交并推送至 `main`（`9cdd444f`），待确认 Vercel 生产部署并发布 Notion 内容
...
| 批次 C：AdSense 与隐私合规           |      5 |      2 |      3 | 🟡 代码已部署，Notion/AdSense 站外配置待完成 |
| 批次 D：内容质量与内部链接           |      5 |      1 |      4 | 🟡 代码已推送，Notion 内容侧进行中           |
| 批次 E：部署、提交与持续观察         |      8 |      1 |      7 | 🟡 A/B/C 已部署，D 待生产确认与在线验收      |
...
| C-01   | C    | 从 GLOBAL_JS 等配置中移除重复 AdSense 脚本                         | 广告代码          | P0     | 🟡 部分完成 | 2026-08-11 | 代码会识别并阻断 `GLOBAL_JS` 中的 `adsbygoogle`/`ca-pub` 注入；线上配置确认当前 `GLOBAL_JS` 仅含一段 AdSense 脚本；保护代码已随 `230d0bf2` 部署                        | 仍需在 Notion 配置中心手工清空该段 `GLOBAL_JS`，再复核页面源代码                                                 |
| C-02   | C    | 建立全站唯一、幂等的 AdSense 加载入口                              | 广告代码          | P0     | ✅ 已完成   | 2026-08-11 | 建立统一 `AdsenseLoader`；兼容 Notion 的 `NEXT_PUBLIC_ADSENSE_*` 旧键；自动化测试 9/9，生产部署白名单验证通过                                                          | 已随 `230d0bf2` 部署                                                                                             |
| C-03   | C    | 设置广告页面白名单并限制自动广告覆盖非内容页                       | 广告策略          | P0     | 🟡 部分完成 | 2026-08-11 | 代码白名单仅允许首页和不少于 600 字的已发布 Post；Page、搜索、404、认证、后台、密码页、薄内容和风险页默认无脚本/广告位；生产 `/search` 与风险文章均为 0 脚本、0 广告位 | 仍需在 AdSense 后台关闭 Auto Ads 或建立同等 URL 排除规则                                                         |
| C-04   | C    | 配置 CMP、欧洲消息和 Consent Mode                                  | AdSense 后台/隐私 | P0     | 🟡 部分完成 | 2026-08-11 | 首个 HTML 已包含 EEA、英国、瑞士地区的 Consent Mode v2 默认拒绝信号；`magzine` 页脚“隐私政策”入口已部署                                                                | 需在 AdSense“隐私与消息”启用 Google 认证 CMP/TCF v2.3，并补写隐私政策更新时间、第三方供应商和 CMP 说明           |
...
| D-01   | D    | 优先完善 6 篇薄内容或低价值文章                                    | Notion 内容       | P1     | 🟡 部分完成 | 2026-08-11 | 生产页基线为约 118–365 字/词，6 篇均无 H2/H3 和正文上下文链接；已在 `BATCH_D_CONTENT_QUALITY_WORKBOOK.md` 完成逐篇补强卡、结构提纲、导语草稿、内链矩阵与官方资料       | 需要在 Notion 中结合一手经验、截图和验证结果完成正文并发布                                                       |
| D-02   | D    | 重写 About 页面，补充作者、资历、站点定位和联系方式                | Notion 内容       | P1     | 🟡 部分完成 | 2026-08-11 | 线上 About 约 477 字/词且为 `noindex`；工作簿已提供可直接改写的完整结构与草稿                                                                                          | 在 Notion 补齐真实作者资料和联系方式，并将状态改为 `Published` 后复核索引指令                                    |
| D-03   | D    | 完善 Contact、Privacy、Terms、Disclaimer、Editorial Policy         | Notion 内容       | P0     | 🟡 部分完成 | 2026-08-11 | 线上隐私政策约 136 字/词；Contact、Terms、Disclaimer、Editorial Policy 均为 404；工作簿已准备五类页面草稿和发布检查项                                                  | 在 Notion 创建/更新页面，确认 slug、Published 状态和页脚链接后逐页验收                                           |
| D-04   | D    | 为分类和标签页补充唯一导语与主题说明                               | Notion 内容       | P1     | ✅ 已完成   | 2026-08-11 | `magzine` 已为三类主分类提供唯一主题说明，为标签页生成去重说明，并增加三篇“建议阅读顺序”；SEO 描述与页面正文复用同一数据源                                             | 自动化测试 11/11、类型检查和生产构建通过；代码已随 `9cdd444f` 推送至 `main`，待生产在线复核                      |
| D-05   | D    | 增加上下文链接、面包屑和相关推荐                                   | 模板/内容         | P1     | 🟡 部分完成 | 2026-08-11 | `magzine` 已增加分类/标签阅读路径；文章面包屑改为“首页 > 分类 > 当前分类 > 文章”；相关推荐按共享标签、共享分类和发布时间排序并补足，避免模块为空                       | 模板代码已随 `9cdd444f` 推送；6 篇正文中的上下文链接仍需按工作簿在 Notion 落地                                   |
| E-01   | E    | 运行 lint、类型检查、自动化测试和生产构建                          | 工程验证          | P0     | ✅ 已完成   | 2026-08-11 | A/B/C/D 均完成针对性测试；批次 D 相关 Jest 16/16、TypeScript、变更文件 ESLint 0 错误和 Next.js 15.5.23 生产构建 50/50 通过                                             | ESLint 仅保留 `themes/magzine/index.js` 两条既有 Hook 依赖警告；全仓库既有 sitemap 测试环境域名差异需另行收敛    |
| E-02   | E    | 部署生产环境并记录版本与回滚点                                     | 部署              | P0     | 🟡 部分完成 | 2026-08-11 | A/B/C 与部署修复已由 Vercel 部署；批次 D 提交 `9cdd444f` 已推送到 GitHub `main`，本地与 `origin/main` 一致                                                             | 待确认 `9cdd444f` 的 Vercel 生产部署状态；确认前沿用已验证的 `230d0bf2` 作为生产回滚点                           |
| E-03   | E    | 在线复核无 JavaScript 正文、canonical、重定向和 sitemap            | 线上验证          | P0     | 🟡 部分完成 | 2026-08-11 | 正文、canonical、主域、HTTPS、`www` sitemap 与文章版式已复核；批次 C 生产首页 1 个 AdSense 主脚本，`/search` 与风险文章均为 0                                          | A-04 所列 `.html`、默认语言和裸域 sitemap 残留项待继续收敛；确认 `9cdd444f` 上线后复核分类导语、面包屑和相关推荐 |
...
| 2026-08-11 | C-01～C-05                   | 重写 AdSense 加载与广告位初始化；阻断 `GLOBAL_JS` 广告注入；加入路由/内容白名单、风险排除、Consent Mode v2 默认值和 `magzine` 隐私入口                      | 代码随 `230d0bf2` 部署；C-02、C-05 完成，C-01、C-03、C-04 的站外配置待执行                                      | 批次 C Jest 9/9；TypeScript；ESLint 0 错误；生产构建 50/50；线上首页 1 个主脚本，`/search` 与风险文章均为 0 | Notion `GLOBAL_JS` 尚未手工清空；AdSense Auto Ads/CMP 无后台权限；线上隐私政策约 136 字/词且披露不完整 | 完成 10.8 站外操作清单，再申请或复审 AdSense                                          |
| 2026-08-11 | D-01～D-05                   | 审计 6 篇薄内容、About、政策页和分类页；实现 `magzine` 分类/标签唯一导语、阅读路径、多层面包屑与相关推荐排序；编制 Notion 内容工作簿                        | D-04 完成；D-01、D-02、D-03、D-05 的代码/草稿侧完成，等待 Notion 内容发布                                       | 生产 DOM 基线审计；批次 D 相关 Jest 16/16；TypeScript；变更文件 ESLint 0 错误；生产构建 50/50               | 当前没有可写 Notion 集成；作者身份、联系方式、一手经验、截图与验证结果不能由代码替代                   | 按 `BATCH_D_CONTENT_QUALITY_WORKBOOK.md` 在 Notion 补写并发布；确认生产部署后在线复核 |
| 2026-08-11 | D GitHub 提交与推送          | 将批次 D 的 13 个代码、测试和文档文件提交到 `main` 并推送 GitHub                                                                                            | 提交 `9cdd444f` 推送成功；`HEAD`、`origin/main` 与 `origin/HEAD` 一致                                           | 暂存差异检查通过；推送后远端引用核对；提交时工作区干净                                                      | Vercel 生产部署状态尚未检查；Notion 内容仍未发布                                                       | 确认 Vercel 部署成功，抽查分类/标签导语、文章面包屑和相关推荐                         |
...
| 2026-08-11 | 批次 C 生产部署                    | `230d0bf2`                   | ✅ Vercel 部署成功                                  | ✅ 正文正常                        | ✅ 原有策略保留                 | ✅ 主域访问正常                                                                  | ✅ 原有策略保留       | ✅ 原有策略保留                     | ✅ 首页主脚本 1；`/search` 和风险文章 0                 | `230d0bf2`            | 代码侧生产验收通过；CMP、Auto Ads 和 Notion 配置仍为站外待办 |
| 2026-08-11 | 批次 D GitHub 版本（生产待确认）   | `9cdd444f`                   | ✅ 本地 50/50 静态页生成完成；Vercel 待确认         | ✅ 原有 SSG 正文保留               | ✅ 分类/标签描述测试通过        | 不涉及                                                                           | ✅ 原有策略保留       | ✅ 文章四层 BreadcrumbList 测试通过 | 不涉及，沿用批次 C 策略                                 | `230d0bf2`            | 已推送 `main`；待确认部署并完成 Notion 内容与生产在线验收    |
...
| 2026-08-11 | 批次 C 代码已部署 | 尚未申请/复审 | 待登录后台检查     | 线上正常 | 代码已有 Consent Mode v2 默认值；Google 认证 CMP 待启用 | 代码白名单已部署；Auto Ads/URL 排除待后台配置 | `/article/1-1-14` 已在代码侧排除，生产页 0 主脚本 | 完成下方站外操作清单后再申请或复审 |
...
| C-M04   | Notion 隐私政策      | 增加最后更新日期；披露 Google 及其他第三方供应商使用 Cookie、个性化/非个性化广告、Google Ads Settings 退出方式、CMP 覆盖地区和联系方式                 | ⬜ 待执行；当前页约 136 字/词，批次 D 工作簿已有草稿 | `/privacy-policy` 可见完整披露，页脚可从全站访问，并与后台实际服务一致                                                          |
| C-M05   | GitHub/Vercel        | 提交并部署本批次代码；逐一抽查首页、安全文章、`/search`、`/privacy-policy`、404、登录页和 `/article/1-1-14`                                            | ✅ 已完成（`230d0bf2`）                              | Vercel 部署成功；线上首页主脚本 1，`/search` 和 `/article/1-1-14` 均为 0；Consent Mode 默认信号存在，正文加载正常               |
...
| 抽样页面原始 HTML 含完整正文比例          |        0%（本次抽样） |                       100% |           100%（本地构建及线上正文抽样） | 已达标       |
| 抽样页面 AdSense 主脚本加载次数           | 约 5 次（代表页抽样） | 白名单页 1 次；排除页 0 次 | 生产首页为 1，`/search` 与风险文章均为 0 | 代码侧已达标 |
